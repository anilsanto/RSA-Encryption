...
static NSData *base64_decode(NSString *inputString){
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    data = [RSAEncryption stripPublicKeyHeader:data];
    if(!data){
        return nil;
    }
    NSString *tag = @"CYBOSOL_INFOTECH";
...
+(NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
    ...
    return [RSAEncryption encryptData:data publicKey:pubKey];
}
...
+ (NSString *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    SecKeyRef keyRef = [RSAEncryption addPublicKey:pubKey];
    if(!keyRef){
        return nil;
    }
